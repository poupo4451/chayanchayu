...
function buildPrompt({ dialogue, genre }) {
  const lines = (dialogue || []).map((d) => `${d.name}: ${d.text}`).join('\n');
...
2. 使用 [Intro] [Verse 1] [Hook] [Verse 2] [Outro] 等段落标记分段（可根据内容适当增减段落）
3. 歌词整体贴合「${genre}」的节奏感和用词习惯
4. 总字数控制在200~400字之间
5. 只输出歌词文本本身，不要输出任何解释、说明或markdown代码块标记`;
}
